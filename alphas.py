...
class Alphas:
    ...
    def tst_alphas(self, quantiles=5, alpha_names=None, measure="close"):
        ...
        for m in alpha_names:
            ...
            fac = pd.read_hdf(os.path.join(DATA_PATH, "Ashare_data/factor_data/{}_{}.h5".format(self.name, m)), key=m)
...
class Alphas101(Alphas):
    ...
    @staticmethod
    def alpha004(underlying):
        # Alpha#4	 (-1 * Ts_Rank(rank(low), 9))
        close = underlying.data["close"].copy()
        return -1 * ts_rank(cs_rank(close), 9)

    @staticmethod
    def alpha005(underlying):
    ...
    @staticmethod
    def alpha023(underlying):
        # Alpha#23	 (((sum(high, 20) / 20) < high) ? (-1 * delta(high, 2)) : 0)
        close = underlying.data["close"].copy()
        high = underlying.data["high"].copy()
        cond = ts_mean(high, 20) < high
        alpha = close.copy(deep=True)  # pd.DataFrame(np.zeros_like(self.close), index=self.close.index, columns=['close'])
        alpha[cond] = -1 * ts_delta(high, 2).fillna(value=0)
        alpha[~cond] = 0
        return alpha

    @staticmethod
    def alpha024(underlying):
    ...
    @staticmethod
    def alpha029(underlying):
        # Alpha#29	 (min(product(rank(rank(scale(log(sum(ts_min(rank(rank((-1 * rank(delta((close - 1),5))))), 2), 1))))), 1), 5) + ts_rank(delay((-1 * returns), 6), 5))
        close = underlying.data["close"].copy()
        ret = close.pct_change()
        return ts_min(cs_rank(cs_rank(scale(log(ts_sum(cs_rank(cs_rank(-1 * cs_rank(ts_rank((close - 1), 5)))), 2)), 1))), 5) + ts_rank(ts_delay((-1 * ret), 6), 5)

    @staticmethod
    def alpha030(underlying):
    ...
    @staticmethod
    def alpha039(underlying):
        # Alpha#39	 ((-1 * rank((delta(close, 7) * (1 - rank(decay_linear((volume / adv20), 9)))))) * (1 +rank(sum(returns, 250))))
        volume = underlying.data["volume"].copy()
        close = underlying.data["close"].copy()
        adv20 = ts_mean(volume, 20)
        ret = close.pct_change()
        return -1 * cs_rank((ts_delta(close, 7) * (1 - cs_rank(ts_decay_linear((volume / adv20), 9))))) * (1 + cs_rank(ts_mean(ret, 250)))

    @staticmethod
    def alpha040(underlying):
    ...
    @staticmethod
    def alpha045(underlying):
        # Alpha#45	 (-1 * ((rank((sum(delay(close, 5), 20) / 20)) * correlation(close, volume, 2)) *rank(correlation(sum(close, 5), sum(close, 20), 2))))
        close = underlying.data["close"].copy()
        volume = underlying.data["volume"].copy()
        df = ts_corr(close, volume, 2)
        df = df.replace([-np.inf, np.inf], 0).fillna(value=0)
        return -1 * (cs_rank(ts_mean(ts_delay(close, 5), 20)) * df * cs_rank(ts_corr(ts_sum(close, 5), ts_sum(close, 20), 2)))

    @staticmethod
    def alpha046(underlying):
    ...
    def alpha047(underlying):
        ...
        vwap = underlying.data["vwap"].copy()
        adv20 = ts_mean(volume, 20)
        return ((((cs_rank((1 / close)) * volume) / adv20) * ((high * cs_rank((high - close))) / (ts_mean(high, 5) / 5))) -
                cs_rank((vwap - ts_delay(vwap, 5))))

    # # Alpha#48	 (indneutralize(((correlation(delta(close, 1), delta(delay(close, 1), 1), 250) *delta(close, 1)) / close), IndClass.subindustry) / sum(((delta(close, 1) / delay(close, 1))^2), 250))

    @staticmethod
    def alpha049(underlying):
    ...
    @staticmethod
    def alpha052(underlying):
        # Alpha#52	 ((((-1 * ts_min(low, 5)) + delay(ts_min(low, 5), 5)) * rank(((sum(returns, 240) -sum(returns, 20)) / 220))) * ts_rank(volume, 5))
        close = underlying.data["close"].copy()
        low = underlying.data["low"].copy()
        volume = underlying.data["volume"].copy()
        ret = close.pct_change()
        return ((-1 * ts_delta(ts_min(low, 5), 5)) * cs_rank(((ts_sum(ret, 240) - ts_sum(ret, 20)) / 220))) * ts_rank(volume, 5)

    @staticmethod
    def alpha053(underlying):
    ...
    def alpha068(underlying):
        ...
        # 后者乘14，使比较双方处于同一水平
        return (ts_rank(ts_corr(cs_rank(high), cs_rank(adv15), 9), 14) < cs_rank(ts_delta(((close * 0.518371) + (low * (1 - 0.518371))), 2)) * 14) * -1
    ...
    def alpha072(underlying):
        ...
        return cs_rank(ts_decay_linear(ts_corr(((high + low) / 2), adv40, 9), 10)) / cs_rank(ts_decay_linear(ts_corr(ts_rank(vwap, 4), ts_rank(volume, 19), 7), 3))

    @staticmethod
    def alpha073(underlying):
    ...
    def alpha074(underlying):
        ...
        return (cs_rank(ts_corr(close, ts_mean(adv30, 37), 15)) < cs_rank(ts_corr(cs_rank(((high * 0.0261661) + (vwap * (1 - 0.0261661)))), cs_rank(volume), 11))) * -1

    @staticmethod
    def alpha075(underlying):
    ...
    @staticmethod
    def alpha078(underlying):
        volume = underlying.data["volume"].copy()
        vwap = underlying.data["vwap"].copy()
        low = underlying.data["low"].copy()
        adv40 = ts_mean(volume, 40)
        return cs_rank(ts_corr(ts_sum(((low * 0.352233) + (vwap * (1 - 0.352233))), 20), ts_sum(adv40, 20), 7)).pow(cs_rank(ts_corr(cs_rank(vwap), cs_rank(volume), 6)))
    ...
    def alpha092(underlying):
        ...
        high = underlying.data["high"].copy()
        adv30 = ts_mean(volume, 30)
        p1 = ts_rank(ts_decay_linear(((((high + low) / 2) + close) < (low + open)), 15), 19)
    ...
    def alpha095(underlying):
        ...
        adv40 = ts_mean(volume, 40)
        # 前者乘以12，使比较双方处于同一水平
        return (cs_rank((open - ts_min(open, 12))) * 12 < ts_rank(
            (cs_rank(ts_corr(ts_mean(((high + low) / 2), 19), ts_mean(adv40, 19), 13)).pow(5)), 12)).astype(
            'int')
    #
    # # Alpha#96	 (max(Ts_Rank(decay_linear(correlation(rank(vwap), rank(volume), 3.83878),4.16783), 8.38151), Ts_Rank(decay_linear(Ts_ArgMax(correlation(Ts_Rank(close, 7.45404),Ts_Rank(adv60, 4.13242), 3.65459), 12.6556), 14.0365), 13.4143)) * -1)
    @staticmethod
    def alpha096(underlying):
        vwap = underlying.data["vwap"].copy()
        close = underlying.data["close"].copy()
        volume = underlying.data["volume"].copy()
        adv60 = ts_mean(volume, 60)
    ...
    @staticmethod
    def alpha098(underlying):
        vwap = underlying.data["vwap"].copy()
        volume = underlying.data["volume"].copy()
        adv5 = ts_mean(volume, 5)
        adv15 = ts_mean(volume, 15)
        return (cs_rank(ts_decay_linear(ts_corr(vwap, ts_mean(adv5, 26), 5), 7)) - cs_rank(
            ts_decay_linear(ts_rank(ts_argmin(ts_corr(cs_rank(open), cs_rank(adv15), 21), 9), 7), 8)))
    #
    # # Alpha#99	 ((rank(correlation(sum(((high + low) / 2), 19.8975), sum(adv60, 19.8975), 8.8136)) <rank(correlation(low, volume, 6.28259))) * -1)
    @staticmethod
    def alpha099(underlying):
        low = underlying.data["low"].copy()
        high = underlying.data["high"].copy()
        volume = underlying.data["volume"].copy()
        adv60 = ts_mean(volume, 60)
...
if __name__ == "__main__":
    ...
    u = Underlying()
    alphas_101.cal_alphas(u, n_jobs=1)
    # alphas_101.tst_alphas()